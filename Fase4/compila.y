...
void AddInstr(OpCode op, int val) {
    prog[ip++] = (INSTR) {op, {.t = NUM, .val.n = val}};
}
%}
...
%token EQt NEt LTt LEt GTt GEt ABRE FECHA SEP PONTO
%token IF WHILE FUNC PRINT AC
...
Expr: NUMt {  AddInstr(PUSH, $1);}
    | SUBt NUMt { /* Para lidar com inteiros negativos */
        AddInstr(PUSH, -1);
        AddInstr(MUL, 0);
    }
    | ID {
    ...
    | AC OPEN Expr SEP Expr CLOSE { AddInstr(SIS, 0); } /* Ação e chamada ao sistema */
;
